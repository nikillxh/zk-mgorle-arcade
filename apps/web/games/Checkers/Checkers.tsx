...
export default function RandzuPage({
  ...
  // Commenting out blockchain-related logic
  /*
  ...
  */
  ...
  useEffect(() => {
    if (matchQueue.pendingBalance && !matchQueue.inQueue) {
      ...
      // collectPending(); // Commented out blockchain-related logic
    }
    if (!walletInstalled()) {
  ...
  const bottomButtonState = {
    [GameState.OpponentTimeout]: {
      ...
      handler: () => {
        // proveOpponentTimeout(); // Commented out blockchain-related logic
      },
    },
    [GameState.Lost]: {
      text: 'RESTART',
